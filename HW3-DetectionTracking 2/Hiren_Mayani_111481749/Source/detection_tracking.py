...
def of(v,file_name):
    ...
    while (1):
        ...
        #cv2.imwrite('img' + str(frameCounter) + '.jpg', img2)
...
def spreadParticles(particles,c,r,w,h):
    area = w*h

    n = len(particles)
    if area > n:
        particle_density = int(area/n)
        i = 0
        newparticles = np.ndarray(n,2)
        print(newparticles)
        for i in range(n):
            #newparticles =
            i+=particle_density
    else:
        particle_density = int(n/area)
...
def particleFilter(v,file_name):
    output_name = sys.argv[3] + file_name
    output = open(output_name, "w")
    frameCounter = 0
...
def kalmanFilter(v,file_name):
    ...
    while (1):
        ...
        if ret == False:
            break
        measurement = kalman.correct()#np.array([[np.float32(c+w/2)],[np.float32(r+h/2)]]))
        print(measurement)
        tp = kalman.predict()

        img2 = cv2.circle(frame, (tp[0], tp[1] ), 5, (0, 255, 0), -1)
        cv2.imwrite('img' + str(frameCounter) + '.jpg', img2)
...
def skeleton_tracker(v, file_name):
    if file_name == "output_camshift.txt":
        camShift(v,file_name)
    elif file_name == "output_particle.txt":
        particleFilter(v,file_name)
    elif file_name == "output_kalman.txt":
        kalmanFilter(v,file_name)
    else:
     of(v,file_name)

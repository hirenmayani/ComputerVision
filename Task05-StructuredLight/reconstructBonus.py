...
def reconstruct_from_binary_patterns():
    scale_factor = 1.0
    ref_white = cv2.resize(cv2.imread("images_new/pattern000.jpg", cv2.IMREAD_GRAYSCALE) / 255.0, (0, 0), fx=scale_factor,
                           fy=scale_factor)
    ref_black = cv2.resize(cv2.imread("images_new/pattern001.jpg", cv2.IMREAD_GRAYSCALE) / 255.0, (0, 0), fx=scale_factor,
                           fy=scale_factor)

    ref_clr = cv2.resize(cv2.imread("images_new/pattern001.jpg", 1) / 255.0, (0, 0), fx=scale_factor,
                           fy=scale_factor)
    ...
    for i in range(0, 15):
        # read the file
        patt = cv2.resize(cv2.imread("images_new/pattern%03d.jpg" % (i + 2), cv2.IMREAD_GRAYSCALE) /255.0, (0, 0), fx=scale_factor, fy=scale_factor)
...
#    print(camera_points)
    ...
    with open("stereo_calibration.pckl", "r") as f:
        ...
        cam = cv2.undistortPoints(np.array(camera_points,dtype=np.float32), camera_K,camera_d)

        # TODO: use cv2.undistortPoints to get normalized points for projector, use projector_K and projector_d
        proj = cv2.undistortPoints(np.array(projector_points,dtype=np.float32), projector_K, projector_d)
...
#        pts = cv2.triangulatePoints(np.ones((3,4)),np.concatenate((projector_R, projector_t), axis=1),cam,proj)
        #np.eye()
        pts = cv2.triangulatePoints(np.concatenate((np.identity(3), np.zeros((3,1))), axis=1),np.concatenate((projector_R, projector_t), axis=1),cam,proj).T
